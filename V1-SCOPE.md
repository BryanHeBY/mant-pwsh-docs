...
| `pwsh-cli` | 150 | Native tools frequently called from PowerShell, including Windows shell/GUI/registry entry points, batch/file, directory, link, text, process, host, network, account/SMB/service compatibility, Remote Desktop session inventory/notification/lifecycle/control, WinRM/WinRS remote management, Event Log/forwarding, performance-counter and ETW diagnostics, system-information/GUI diagnostics, cleanup, shutdown, time-zone, Driver Verifier, installation, image-servicing, driver, security-policy, authentication, ACL, EFS, certificate, advanced filesystem and disk management, VSS and backup, ReFS diagnosis/recovery, compression, BitLocker, volume mounting, and boot-recovery operations, file associations, winget, and optional Microsoft Learn MCP discovery. |
| **Total** | **210** | Current first-release document inventory. |
...
- all 210 manifest documents exist and pass portable validation;
