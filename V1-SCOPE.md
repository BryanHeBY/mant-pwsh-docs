...
| `pwsh-cli` | 120 | Native tools frequently called from PowerShell, including Windows shell/GUI/registry entry points, batch/file, directory, link, text, process, host, network, installation, image-servicing, driver, security-policy, authentication, ACL, EFS, certificate, advanced filesystem and disk management, VSS and backup, ReFS diagnosis/recovery, compression, BitLocker, volume mounting, and boot-recovery operations, file associations, winget, and optional Microsoft Learn MCP discovery. |
| **Total** | **180** | Current first-release document inventory. |
...
- all 180 manifest documents exist and pass portable validation;
