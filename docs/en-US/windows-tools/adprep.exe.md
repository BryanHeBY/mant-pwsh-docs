...
`Get-Item -LiteralPath "{{D:\support\adprep\adprep.exe}}" | Select-Object FullName, Length, @{Name='ProductVersionFixed';Expression={$_.VersionInfo.ProductVersionRaw.ToString()}}, @{Name='ProductVersionString';Expression={$_.VersionInfo.ProductVersion}}`
...
`adprep.exe` Application candidate under either PowerShell collector. No
...
Behavior verification remains approved-media binary/signature and
read-only FSMO/schema inventory only; no forest, domain, GPO or RODC
preparation, silent execution, schema/role bypass or retry is permitted merely
for evidence.
