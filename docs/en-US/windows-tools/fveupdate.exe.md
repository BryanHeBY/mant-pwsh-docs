...
`fveupdate.exe` Application candidate under either PowerShell collector. No
...
Behavior verification remains file metadata, manage-bde status/protector
inventory and event reading only; no FveUpdate execution, protector,
encryption, suspension, volume, setup, recovery or reboot mutation is permitted
merely for evidence.

## Related documents
- [manage-bde.exe](manage-bde.exe.md)
- [bdehdcfg.exe](bdehdcfg.exe.md)
