...
`format.com` creates a new filesystem on a volume, mount point, or drive
...
- `format.com`, `format`: Destructively create a new filesystem on one explicitly
...
Windows ships the formatter as `format.com`, not `format.exe`. PowerShell can
resolve functions, aliases, scripts, and other applications before it reaches
the system tool; bare `format` can therefore be shadowed by a `format.bat` or
another earlier match. Invoke `format.com` explicitly, quote mount-point paths,
and never construct a target from unvalidated text or a first-match pipeline.
...
use installed `format.com /?` and the applicable filesystem guidance.
...
`10.0.26100.1`; exact `format.exe` discovery found no command. Bare `format`
also returned third-party `format.bat` candidates, proving that destructive
automation must resolve the `.com` image explicitly. `format.com /?` printed
88 help lines and returned 0 without a drive, volume, or mount point. No
format, dismount, overwrite, label, filesystem, allocation-unit, or media
mutation ran; operation behavior remains restricted to a disposable storage
fixture with independent recovery.
