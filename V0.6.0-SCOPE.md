...
Version 0.6.0 is the first publishable English reference release. It provides a
...
is [`release/v0.6.0.json`](release/v0.6.0.json).
...
| `windows-tools` | 350 | Windows-native and optional executables, Cmd builtins and interactive families, shell/GUI/URI entry points, package and system management, diagnostics, server roles, compatibility and recovery utilities, and optional Microsoft Learn MCP discovery. |
| `cross-platform-tools` | 6 | A dedicated index plus Git, OpenSSH, curl, tar, and .NET CLI guidance for Windows, macOS, and Linux. |
| **Total** | **416** | Current first-release document inventory. |
...
The `v0.6.0` tag may be created only when:

- all 416 manifest documents exist and pass portable validation;
...
## Deferred from version 0.6.0

Version 0.6.0 does not duplicate upstream pages one-for-one. The Windows tool
