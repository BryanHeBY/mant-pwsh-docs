# Version 0.7.0 runtime verification

This checklist records the runtime evidence required by
[V0.7.0-SCOPE.md](../V0.7.0-SCOPE.md) before creating the final `v0.7.0` tag. It is separate
...
On Windows, run the protected Cmd behavior fixture through both installed
PowerShell collectors before using its results for Cmd pages:
...
Require 17/17 checks from each run and confirm the reported temporary roots no
longer exist afterward. Record both collector versions and the fixed System32
`cmd.exe` version.

Also compare the independently serviced WinGet client help with the current
working-tree entries through both collectors:

```powershell
powershell.exe -NoProfile -File .\tests\runtime\audit-winget-option-coverage.ps1 -FailOnDifference
pwsh -NoProfile -File .\tests\runtime\audit-winget-option-coverage.ps1 -FailOnDifference
```

Record the WinGet version, exact resolved path, option counts, and any drift.
This invokes only `--version` and subcommand `--help`; it does not inventory or
change sources, packages, or the network.
...
On Windows, first record whether the shell token is ordinary or elevated and
run each query in the least-privileged context its interface supports. A
read-only verb is not necessarily available to an ordinary token: BCDEdit,
Manage-Bde, REAgentC, and VssAdmin require elevation for the system-state
queries below, while AuditPol `/get` requires delegated policy-object Read
permission or `SeSecurityPrivilege`. Preserve complete native diagnostics and
`$LASTEXITCODE`; an access failure is not an empty, absent, disabled, or
unsupported resource. Run and record supported versions of:

```powershell
winget --version
$wslPath = Join-Path $env:SystemRoot 'System32\wsl.exe'
if (Test-Path -LiteralPath $wslPath -PathType Leaf) {
    Get-Item -LiteralPath $wslPath |
        Select-Object FullName,
            @{Name='FileVersionFixed';Expression={$_.VersionInfo.FileVersionRaw.ToString()}},
            @{Name='FileVersionString';Expression={$_.VersionInfo.FileVersion}}
    & $wslPath --version
    $wslVersionExit = $LASTEXITCODE
    & $wslPath --status
    $wslStatusExit = $LASTEXITCODE
    [pscustomobject]@{ VersionExit = $wslVersionExit; StatusExit = $wslStatusExit }
} else {
    [pscustomobject]@{ Path = $wslPath; Available = $false }
}
where.exe winget
robocopy.exe /?
...
net.exe start
net.exe view
$rdsQueryTools = @('query.exe', 'quser.exe', 'qwinsta.exe', 'qprocess.exe', 'qappsrv.exe')
foreach ($rdsQueryTool in $rdsQueryTools) {
    $rdsQueryPath = "$env:SystemRoot\System32\$rdsQueryTool"
    if (-not (Test-Path -LiteralPath $rdsQueryPath -PathType Leaf)) {
        "Record $rdsQueryTool as not installed; do not substitute a PATH match."
        continue
    }
    switch ($rdsQueryTool) {
        'query.exe' { & $rdsQueryPath user; & $rdsQueryPath session; & $rdsQueryPath process '*'; & $rdsQueryPath session /counter }
        'qprocess.exe' { & $rdsQueryPath '*' }
        'qappsrv.exe' { & $rdsQueryPath /continue }
        default { & $rdsQueryPath }
    }
}
$rdsSessionTools = @(
    'msg.exe', 'tsdiscon.exe', 'logoff.exe', 'rwinsta.exe', 'tscon.exe',
    'shadow.exe', 'tskill.exe', 'change.exe', 'chglogon.exe', 'chgport.exe',
    'chgusr.exe', 'flattemp.exe'
)
foreach ($rdsSessionTool in $rdsSessionTools) {
    $rdsSessionPath = "$env:SystemRoot\System32\$rdsSessionTool"
    if (-not (Test-Path -LiteralPath $rdsSessionPath -PathType Leaf)) {
        "Record $rdsSessionTool as not installed; do not substitute a PATH match."
        continue
    }
    switch ($rdsSessionTool) {
        'change.exe' { & $rdsSessionPath logon /query; & $rdsSessionPath port /query; & $rdsSessionPath user /query }
        'chglogon.exe' { & $rdsSessionPath /query }
        'chgport.exe' { & $rdsSessionPath /query }
        'chgusr.exe' { & $rdsSessionPath /query }
        'flattemp.exe' { & $rdsSessionPath /query }
        default { & $rdsSessionPath /? }
    }
}
foreach ($rdsPolicyTool in @('tsecimp.exe', 'tsprof.exe')) {
    $rdsPolicyPath = "$env:SystemRoot\System32\$rdsPolicyTool"
    if (-not (Test-Path -LiteralPath $rdsPolicyPath -PathType Leaf)) {
        "Record $rdsPolicyTool as not installed; do not substitute a PATH match."
        continue
    }
    if ($rdsPolicyTool -eq 'tsecimp.exe') { & $rdsPolicyPath /d }
    else { & $rdsPolicyPath /q /local "$approvedLocalRdsTestUser" }
}
Get-Command winrm, winrm.cmd, winrm.exe, winrm.vbs -All -ErrorAction SilentlyContinue
...
powercfg.exe /list
# Run /requests from an elevated shell; preserve access failure separately from an empty result.
...
Get-Item -LiteralPath $mrtPath | Select-Object FullName, Length, LastWriteTime, VersionInfo
Get-AuthenticodeSignature -LiteralPath $mrtPath
Get-FileHash -LiteralPath $mrtPath -Algorithm SHA256
# Do not invoke MRT merely for help evidence: /? opens a GUI, while scan modes can log/report/clean/restart.
...
Get-Item -LiteralPath $wscriptPath, $regsvr32Path -ErrorAction SilentlyContinue |
    Select-Object FullName, Length,
        @{Name='FileVersionFixed'; Expression={$_.VersionInfo.FileVersionRaw.ToString()}},
        @{Name='FileVersionString'; Expression={$_.VersionInfo.FileVersion}}
# WScript help is GUI output, and RegSvr32 operation loads a supplied component.
# Do not invoke either merely to turn a dialog or component-load path into evidence.
Get-WindowsCapability -Online | Where-Object Name -Like 'WMIC*'
mstsc.exe /?
mstsc.exe /l
sxstrace.exe -?
Get-Command format, format.com, format.exe, recover.exe, pktmon.exe, tpmtool.exe, makecab.exe, diantz.exe, expand.exe -All -ErrorAction SilentlyContinue
format.com /?
recover.exe /?
pktmon.exe status
...
makecab.exe /?
$diantzPath = "$env:SystemRoot\System32\diantz.exe"
if (Test-Path -LiteralPath $diantzPath -PathType Leaf) {
    & $diantzPath /?
} else {
    'Record diantz.exe as not installed; do not substitute a PATH match.'
}
expand.exe /?
...
$cmdBuiltinHelpNames = @(
    'CMD', 'SET', 'SETLOCAL', 'CALL', 'ENDLOCAL', 'EXIT', 'FOR', 'IF',
    'PATH', 'PUSHD', 'POPD', 'TITLE', 'ASSOC', 'FTYPE', 'DIR', 'COPY',
    'DEL', 'ERASE', 'RD', 'RMDIR', 'CD', 'CHDIR', 'MOVE', 'MD', 'MKDIR',
    'REN', 'RENAME', 'TYPE', 'MKLINK'
)
foreach ($cmdBuiltinHelpName in $cmdBuiltinHelpNames) {
    cmd.exe /d /c help $cmdBuiltinHelpName
    # Complete built-in help commonly returns 1; preserve stdout and status.
}
cmd.exe /d /c "date /t"
cmd.exe /d /c "time /t"
...
at.exe /?
# Current builds may return deprecation/binding failure rather than inventory; retain output and status.
at.exe; $atInventoryExitCode = $LASTEXITCODE
cacls.exe /?
icacls.exe /?
takeown.exe /?
taskkill.exe /?
$bootCfgPath = "$env:SystemRoot\System32\bootcfg.exe"
if (Test-Path -LiteralPath $bootCfgPath -PathType Leaf) {
    & $bootCfgPath /?
    & $bootCfgPath /query
} else {
    'Record bootcfg.exe as not installed; do not substitute a PATH match.'
}
...
$legacyNetworkTools = @('ftp.exe', 'tftp.exe', 'telnet.exe', 'finger.exe')
foreach ($legacyNetworkTool in $legacyNetworkTools) {
    $legacyNetworkPath = "$env:SystemRoot\System32\$legacyNetworkTool"
    if (-not (Test-Path -LiteralPath $legacyNetworkPath -PathType Leaf)) {
        "Record $legacyNetworkTool as not installed; do not substitute a PATH match."
        continue
    }
    if ($legacyNetworkTool -eq 'ftp.exe') { & $legacyNetworkPath -? }
    else { & $legacyNetworkPath /? }
}
Get-WindowsOptionalFeature -Online -FeatureName TFTP
Get-WindowsOptionalFeature -Online -FeatureName TelnetClient
$nfsRpcTools = @('nfsadmin.exe', 'nfsshare.exe', 'nfsstat.exe', 'showmount.exe', 'rpcinfo.exe', 'rpcping.exe', 'mount.exe', 'umount.exe')
foreach ($nfsRpcTool in $nfsRpcTools) {
    $nfsRpcPath = "$env:SystemRoot\System32\$nfsRpcTool"
    if (-not (Test-Path -LiteralPath $nfsRpcPath -PathType Leaf)) {
        "Record $nfsRpcTool as not installed; do not substitute a PATH match."
        continue
    }
    switch ($nfsRpcTool) {
        'nfsadmin.exe' { & $nfsRpcPath server config; & $nfsRpcPath server -l; & $nfsRpcPath client config }
        'nfsshare.exe' { & $nfsRpcPath }
        'nfsstat.exe' { & $nfsRpcPath; & $nfsRpcPath -m }
        'showmount.exe' { & $nfsRpcPath -e $approvedNfsServer }
        'rpcinfo.exe' { & $nfsRpcPath /p $approvedNfsServer }
        'rpcping.exe' { & $nfsRpcPath /?; & $nfsRpcPath /t ncacn_ip_tcp /s $approvedRpcServer /i 1 /v 2 }
        'mount.exe' { & $nfsRpcPath }
        'umount.exe' { & $nfsRpcPath /? }
    }
}
$directoryTools = @('dcdiag.exe', 'repadmin.exe', 'nltest.exe', 'netdom.exe')
foreach ($directoryTool in $directoryTools) {
    $directoryPath = "$env:SystemRoot\System32\$directoryTool"
    if (-not (Test-Path -LiteralPath $directoryPath -PathType Leaf)) {
        "Record $directoryTool as not installed; do not substitute a PATH match."
        continue
    }
    if ($directoryTool -eq 'netdom.exe') { & $directoryPath help }
    else { & $directoryPath /? }
}
$dcDiagPath = "$env:SystemRoot\System32\dcdiag.exe"
if (Test-Path -LiteralPath $dcDiagPath -PathType Leaf) {
    & $dcDiagPath /s:"dc01.example.com" /test:Connectivity /v
    & $dcDiagPath /s:"dc01.example.com" /test:DNS /DnsBasic /v
    & $dcDiagPath /s:"dc01.example.com" /test:Replications /v
}
$repAdminPath = "$env:SystemRoot\System32\repadmin.exe"
if (Test-Path -LiteralPath $repAdminPath -PathType Leaf) {
    & $repAdminPath /replsummary "dc01.example.com" /bysrc /bydest
    & $repAdminPath /showrepl "dc01.example.com" /all /verbose
    & $repAdminPath /queue "dc01.example.com"
}
...
if (Test-Path -LiteralPath $netDomPath -PathType Leaf) {
    & $netDomPath query /domain:"example.com" DC
    & $netDomPath query /domain:"example.com" FSMO
    & $netDomPath query /domain:"example.com" TRUST
    & $netDomPath verify "member01.example.com" /domain:"example.com"
}
foreach ($optionalAdTool in @('dcgpofix.exe', 'dcpromo.exe', 'gpfixup.exe')) {
    $optionalAdPath = "$env:SystemRoot\System32\$optionalAdTool"
    if (-not (Test-Path -LiteralPath $optionalAdPath -PathType Leaf)) {
        "Record $optionalAdTool as not installed; do not substitute a PATH match."
        continue
    }
    if ($optionalAdTool -eq 'dcpromo.exe') { & $optionalAdPath '/?:Promotion' }
    else { & $optionalAdPath /? }
}
...
$dnsCmdPath = "$env:SystemRoot\System32\dnscmd.exe"
if (Test-Path -LiteralPath $dnsCmdPath -PathType Leaf) {
    & $dnsCmdPath "dns01.example.com" /enumzones
    & $dnsCmdPath "dns01.example.com" /zoneinfo "example.com"
    & $dnsCmdPath "dns01.example.com" /enumrecords "example.com" "www" /detail
    & $dnsCmdPath "dns01.example.com" /enumdirectorypartitions
}
$dfsDiagPath = "$env:SystemRoot\System32\dfsdiag.exe"
if (Test-Path -LiteralPath $dfsDiagPath -PathType Leaf) {
    & $dfsDiagPath /testsites /machine:"fileserver01.example.com"
    & $dfsDiagPath /testdfsconfig /DFSRoot:"\\example.com\shares"
    & $dfsDiagPath /testdfsintegrity /DFSRoot:"\\example.com\shares"
    & $dfsDiagPath /testreferral /DFSPath:"\\example.com\shares\team" /full
}
...
$ntfrsUtlPath = "$env:SystemRoot\System32\ntfrsutl.exe"
if (Test-Path -LiteralPath $ntfrsUtlPath -PathType Leaf) {
    & $ntfrsUtlPath version "dc01.example.com"
    & $ntfrsUtlPath sets "dc01.example.com"
    & $ntfrsUtlPath poll "dc01.example.com"
}
...
```powershell
$optionalLegacyTools = @('lpq.exe', 'lpr.exe', 'rexec.exe', 'rsh.exe', 'tlntadmn.exe')
foreach ($optionalLegacyTool in $optionalLegacyTools) {
    $optionalLegacyPath = "$env:SystemRoot\System32\$optionalLegacyTool"
    if (Test-Path -LiteralPath $optionalLegacyPath -PathType Leaf) {
        & $optionalLegacyPath /?
    } else {
        "Record $optionalLegacyTool as not installed; do not substitute a PATH match."
    }
}
```
...
Before any credential, policy, or directory query, the following help-only
probe records output and native status separately. Nonzero help status is not
an operational failure, and a successful help display is not necessarily a
complete capability inventory:

```powershell
$helpCases = @(
    @{ File = "$env:SystemRoot\System32\cmdkey.exe"; Arguments = @('/?') }
    @{ File = "$env:SystemRoot\System32\gpupdate.exe"; Arguments = @('/?') }
    @{ File = "$env:SystemRoot\System32\gpresult.exe"; Arguments = @('/?') }
    @{ File = "$env:SystemRoot\System32\setspn.exe"; Arguments = @('/?') }
)

$helpEvidence = foreach ($case in $helpCases) {
    $output = @(& $case.File @($case.Arguments) 2>&1)
    [pscustomobject]@{
        File       = $case.File
        FileVersionFixed = (Get-Item -LiteralPath $case.File).VersionInfo.FileVersionRaw.ToString()
        FileVersionString = (Get-Item -LiteralPath $case.File).VersionInfo.FileVersion
...
Preserve `$helpEvidence` without flattening the per-command exit codes. Compare
GpResult's installed help with the current official reference before deciding
whether `/x`, `/h`, or `/f` is unavailable on that build. Run the stateful or
sensitive inventory below only on an approved non-production host and context.

```powershell
tasklist.exe /fi "PID eq $PID" /fo list
$tasklistHelp = @(tasklist.exe /?)
if ($tasklistHelp -match '(?i)/APPS') { tasklist.exe /apps /fo csv } else { 'Record /APPS as unavailable on this build.' }
...
if (Test-Path -LiteralPath $msiExecPath -PathType Leaf) {
    Get-Item -LiteralPath $msiExecPath | Select-Object FullName, Length, VersionInfo
    'Do not invoke MSIExec merely for help; use only an approved disposable MSI fixture.'
}
...
pwlauncher.exe /?
ktmutil.exe
$elevationGatedHelp = 'winsat.exe', 'tcmsetup.exe'
foreach ($tool in $elevationGatedHelp) {
    $toolPath = "$env:SystemRoot\System32\$tool"
    Get-Item -LiteralPath $toolPath -ErrorAction SilentlyContinue |
        Select-Object FullName, Length,
            @{Name='FileVersionFixed'; Expression={$_.VersionInfo.FileVersionRaw.ToString()}},
            @{Name='FileVersionString'; Expression={$_.VersionInfo.FileVersion}}
}
# Ordinary-token process creation was denied for WinSAT/TcmSetup help on the
# recorded host. Do not synthesize an exit code; run help only in an approved
# elevated diagnostic session, and never supply workload/configuration operands.
$wdsUtilPath = "$env:SystemRoot\System32\wdsutil.exe"
if (Test-Path -LiteralPath $wdsUtilPath -PathType Leaf) {
    & $wdsUtilPath /?
    & $wdsUtilPath /Get-Server /Server:"wds01.example.com" /Show:Config
    & $wdsUtilPath /Verbose /Get-Server /Server:"wds01.example.com" /Show:Images /Detailed
} else {
    'Record wdsutil.exe as not installed; do not substitute a PATH match.'
}
dispdiag.exe -?
msdt.exe /?
$dtracePath = "$env:SystemRoot\System32\dtrace.exe"
if (Test-Path -LiteralPath $dtracePath -PathType Leaf) {
    & $dtracePath -V
    & $dtracePath -l -P syscall
} else {
    'Record dtrace.exe as not installed; do not substitute a PATH match.'
}
...
if (Test-Path -LiteralPath $ktpass -PathType Leaf) {
    & $ktpass /?
} else {
    'Record KtPass as not installed; do not substitute a PATH match.'
}
foreach ($optionalTool in @('rdpsign.exe', 'scwcmd.exe')) {
    $optionalPath = "$env:SystemRoot\System32\$optionalTool"
    if (Test-Path -LiteralPath $optionalPath -PathType Leaf) {
        & $optionalPath /?
    } else {
        "Record $optionalTool as not installed; do not substitute a PATH match."
    }
}
cmdkey.exe /list
gpresult.exe /scope user /r
...
chkntfs.exe C:
chkntfs.exe /t
$autoChkPath = Join-Path $env:SystemRoot 'System32\autochk.exe'
Get-Item -LiteralPath $autoChkPath | Select-Object FullName, Length, LastWriteTime, VersionInfo
Get-AuthenticodeSignature -LiteralPath $autoChkPath
Get-FileHash -LiteralPath $autoChkPath -Algorithm SHA256
# Microsoft explicitly forbids direct command-line invocation of Autochk; identity-only evidence is intentional.
convert.exe /?
defrag.exe C: /A /U /V
compact.exe /?
...
$diskShadowPath = "$env:SystemRoot\System32\diskshadow.exe"
if (Test-Path -LiteralPath $diskShadowPath -PathType Leaf) {
    & $diskShadowPath /?
} else {
    'Record diskshadow.exe as not installed; do not substitute a PATH match.'
}
wbadmin.exe get status
wbadmin.exe get disks
wbadmin.exe get versions
$bdeHdCfgPath = "$env:SystemRoot\System32\bdehdcfg.exe"
if (Test-Path -LiteralPath $bdeHdCfgPath -PathType Leaf) {
    & $bdeHdCfgPath /?
    & $bdeHdCfgPath -driveinfo C:
} else {
    'Record bdehdcfg.exe as not installed; do not substitute a PATH match.'
}
refsutil.exe
```
...
Do not assume `refsutil.exe <family> /?` is help: family parsers vary and can
enter privilege or operation paths even without a volume. Begin with the bare
top-level family list and review exact installed family syntax before any
additional call.
...
or retrim storage, convert FAT/FAT32 to NTFS, dismount a conversion target,
weaken converted ACLs, or change NTFS/CompactOS compression merely to satisfy
this checklist. `convert.exe` verification stops after exact System32 identity
and `/?`; never supply a volume merely for evidence. Treat a read-only CHKDSK
finding on an active volume as evidence to investigate, not authorization for
automatic repair.
...
verification. Repair-Bde verification is help-only: never supply an input
volume, output volume/image, key, password, key package, log path, or `-Force`
merely for evidence. Interpret `-protectionaserrorlevel` 0/1 as protected/unprotected
...
notifications enabled, not proof that a particular device honored TRIM. Keep
BypassIO and DAX to queries, StorageReserve to query/find, and Trace to
top-level inventory/query; do not discover Trace syntax by omitting operands
from `start` or `stop`.
...
Keep Regsvr32 verification to exact executable identity unless a disposable UI/
component fixture is separately approved; do not load/register/unregister/install
any DLL/OCX merely for evidence. Record WMIC executable and FoD availability without
...
media. An ordinary-token bare KtmUtil call can return only an elevation diagnostic;
keep elevated verification to help/list operations and do not resolve, force or
forget a transaction merely for evidence. Query page-file configuration, runtime usage and
...
create/destroy a card or expose PIN/PUK/admin keys. PwLauncher has a no-parameter
state query, not `/query`; do not change it or reboot for evidence. Keep RegIni
to no-operand usage/export/ACL/input/hash review. WinSAT help itself requires an
elevated approved diagnostic context on the recorded host; workloads require a
disposable idle benchmark host and trusted media.
...
open untrusted phone books. Keep TapiCfg to `show`; TcmSetup help itself requires
an elevated approved diagnostic context on the recorded host. Keep its other
evidence to domain/DNS inventory; do not mutate AD partitions/SCPs/defaults or
TAPI server lists.
...
On Windows, macOS, and Linux where declared by the document, resolve an
application first and record the selected implementation. Do not let a
PowerShell alias such as Windows PowerShell 5.1 `curl` stand in for the native
tool, and record an unavailable application without invoking a placeholder:
...
foreach ($pair in $toolProbes.GetEnumerator()) {
    $applications = @(Get-Command $pair.Key -CommandType Application -All -ErrorAction SilentlyContinue)
    if ($applications.Count -eq 0) {
        "Record $($pair.Key) as unavailable; do not claim runtime verification."
        continue
    }
    $applications | Select-Object Name, Source, Version
    $probeArguments = @($pair.Value)
    & $applications[0].Source @probeArguments
    [pscustomobject]@{Name=$pair.Key; Path=$applications[0].Source; ExitCode=$LASTEXITCODE}
}
```
...
client. Some valid version output, including OpenSSH `-V`, can be written to
stderr with exit 0 and appear after later stdout when displays merge concurrent
streams; preserve stream, status and capture timestamps separately. Do not claim an
unavailable command has passed; record it as an
environmental exception and decide whether the 0.7.0 scope or test host must
change.
